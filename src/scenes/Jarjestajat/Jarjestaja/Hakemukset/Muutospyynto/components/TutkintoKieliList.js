...
import {
  handleTutkintoKieliCheckboxChange,
  getTutkintokieliBy
} from "../../../../../../services/koulutukset/koulutusUtil";
...
const TutkintoKieliList = props => {
  ...
  const removeChanges = (muutokset, editValues, fields, alaKoodiArvo) => {
    let removals = [];
    ...
    removals.forEach(removal => {
      _.remove(editValues, value => {
        return (
          value.koodiarvo === removal.koodiarvo && removal.ala === alaKoodiArvo
        );
      });
    });
  };
    ...
    <Wrapper>
      <Heading onClick={this.toggleTutkintoList.bind(this)}>
        ...
        {muutokset.length > 0 ? (
          <SpanMuutos>
            Muutokset:&nbsp;
            <Span color={COLORS.OIVA_PURPLE}>{muutokset.length}</Span>
            <Span
              onClick={event => {
                event.stopPropagation();
                this.removeChanges(muutokset, editValues, fields, alaKoodiArvo);
              }}
            >
              Poista muutokset
            </Span>
          ...
          {_.map(koulutustyypit, (koulutustyyppi, i) => {
            return (
              <KoulutustyyppiWrapper key={i}>
                <Koulutustyyppi>
                  {parseLocalizedField(koulutustyyppi.metadata)}
                </Koulutustyyppi>
                {_.map(koulutustyyppi.koulutukset, (koulutus, i) => {
                  const identifier = `input-tutkintokieli-2-${
                    koulutus.koodiArvo
                  }`;
                  const nimiObjekti = _.filter(koulutus.metadata, m => {
                    return m.kieli === "FI";
                  });
                  ...
                  if (!tutkinto) {
                    tutkinto = _.find(tutkinnotJaKoulutuksetValue, t => {
                      return (
                        t.koodiarvo === koulutus.koodiArvo &&
                  ...
                  if (
                    !tutkinto ||
                    _.find(tutkinnotJaKoulutuksetValue, t => {
                      return (
                        t.koodiarvo === koulutus.koodiArvo &&
                  ...
                  tutkinto["koulutusalaKoodiArvo"] =
                    koulutus.koulutusalaKoodiArvo;
                  tutkinto["koulutustyyppiKoodiArvo"] =
                    koulutus.koulutustyyppiKoodiArvo;
