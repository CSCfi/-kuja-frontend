      ...
      <div>
        <Row>
          {_.map(tutkintomaaraykset, (koulutusala, i) => {
            ...
            let nimiText = nimi;
            let arrays = _.flatten(
              _.concat(
                _.map(koulutusalat, ala => {
                  return ala.koulutukset;
                })
              )
            );
            let koulutuslista = koulutukset.koulutusdata
              ? koulutukset.koulutusdata[koodiarvo].koulutukset
              : [];
...
export default MuutospyyntoWizardTutkintokielet;
