export const SET_BACKEND_CHANGES = "SET_BACKEND_CHANGES";
export const SET_SECTION_DATA = "SET_SECTION_DATA";
export const SAVE_MUUTOSPYYNTO_START = "SAVE_MUUTOSPYYNTO_START";
export const SAVE_MUUTOSPYYNTO_SUCCESS = "SAVE_MUUTOSPYYNTO_SUCCESS";
export const SAVE_MUUTOSPYYNTO_FAILURE = "SAVE_MUUTOSPYYNTO_FAILURE";
export const SEND_MUUTOSPYYNTO_START = "SEND_MUUTOSPYYNTO_START";
export const SEND_MUUTOSPYYNTO_SUCCESS = "SEND_MUUTOSPYYNTO_SUCCESS";
export const SEND_MUUTOSPYYNTO_FAILURE = "SEND_MUUTOSPYYNTO_FAILURE";
export const DOWNLOAD_ATTACHMENT_START = "DOWNLOAD_ATTACHMENT_START";
export const DOWNLOAD_ATTACHMENT_SUCCESS = "DOWNLOAD_ATTACHMENT_SUCCESS";
export const DOWNLOAD_ATTACHMENT_FAILURE = "DOWNLOAD_ATTACHMENT_FAILURE";
