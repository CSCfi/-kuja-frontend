...
import {
  meta_kuljettaja_jatko,
  meta_kuljettaja_perus,
  meta_oppisopimus,
  meta_tyovoima,
  meta_vaativa,
  meta_vankila
} from "../../scenes/Jarjestajat/Jarjestaja/Hakemukset/Muutospyynto/modules/lisaperusteluUtil";
import { getKohdeByTunniste, getMaaraystyyppiByTunniste } from "../../utils/muutospyyntoUtil";
...
export function getKieliByKoodi(koodi, kielet) {
  if (kielet && kielet.data) {
    return _.find(kielet.data, kieli => {
      return kieli.koodiArvo === koodi;
    });
  } else {
    return undefined;
  }
...
export function getKoulutusalaByKoodiarvo(koodi, koulutusalat) {
  if (koulutusalat && koulutusalat.data) {
    return _.find(koulutusalat.data, a => {
      return a.koodiArvo === koodi;
    });
  } else {
    return undefined;
  }
...
export function getKoulutustyyppiByKoodiarvo(koodi, koulutustyypit) {
  if (koulutustyypit && koulutustyypit.data) {
    return _.find(koulutustyypit.data, t => {
      return t.koodiArvo === koodi;
    });
  } else {
    return undefined;
  }
...
export function getTutkintoNimiByKoodiarvo(koodi, koulutukset) {
  if (koulutukset && koulutukset.koulutusdata) {
    let nimi = undefined;
    _.forEach(koulutukset.koulutusdata, ala => {
...
export function getTutkintoByKoodiarvo(koodi, koulutukset) {
  if (koulutukset && koulutukset.koulutusdata) {
...
export function getTutkintoKoodiByMaaraysId(maaraysId, lupa) {
  if (lupa && lupa.data && lupa.data.maaraykset) {
    const { maaraykset } = lupa.data;
...
export function getTutkintoNimiByMaaraysId(maaraysId, lupa) {
  if (lupa && lupa.data && lupa.data.maaraykset) {
    const { maaraykset } = lupa.data;
    const obj = _.find(maaraykset, { id: maaraysId });
...
export function getMuutosperusteluObjectById(
  muutosperustelukoodiarvo,
  muutosperustelut
) {
  if (muutosperustelut && muutosperustelut.data) {
    return _.find(muutosperustelut.data, mperustelu => {
      return String(mperustelu.koodiArvo) === String(muutosperustelukoodiarvo);
    });
  }
}

export function getPaatoskierrosByUuid(paatoskierrosUuid, paatoskierrokset) {
  if (paatoskierrokset && paatoskierrokset.data) {
    return _.find(paatoskierrokset.data, paatoskierros => {
...
export function handleSimpleCheckboxChange(
  event,
  editValues,
  fields,
  isInLupa,
      ...
      fields.push({
        ...
        metadata: {
          kuvaus: {
            ...
            SV:
              "Examina och utbildning får därtill ordnas även på annat håll i Finland, med undantag för landskapet Åland."
          }
        }
      });
    }
...
export function handleCheckboxChange(
  ...
  const sisaltaa_merkityksen = parseLocalizedField(
    metadata,
    "FI",
    "sisaltaaMerkityksen"
  );
  ...
  if (!kohde) {
    if (
      koodistoUri === KOODISTOT.KOULUTUS ||
      koodistoUri === KOODISTOT.KULJETTAJAKOULUTUS ||
      koodistoUri === KOODISTOT.OIVA_TYOVOIMAKOULUTUS ||
      koodistoUri === KOODISTOT.AMMATILLISEEN_TEHTAVAAN_VALMISTAVA_KOULUTUS
    ) {
    ...
    if (isInLupa) {
      ...
      fields.push({
        ...
        meta: {
          ...
          ...(koodistoUri === KOODISTOT.KULJETTAJAKOULUTUS &&
            sisaltaa_merkityksen === "jatko" && {
              perusteluteksti_kuljetus_jatko: meta_kuljettaja_jatko
            }),
          ...(koodistoUri === KOODISTOT.KULJETTAJAKOULUTUS &&
            sisaltaa_merkityksen === "perus" && {
              perusteluteksti_kuljetus_perus: meta_kuljettaja_perus
            }),
          ...(koodistoUri === KOODISTOT.OIVA_MUUT &&
            kasite === "laajennettu" && {
              perusteluteksti_oppisopimus: meta_oppisopimus
            }),
          ...(koodistoUri === KOODISTOT.OIVA_MUUT &&
            kasite === "vankila" && { perusteluteksti_vankila: meta_vankila }),
          ...(koodistoUri === KOODISTOT.OIVA_MUUT &&
            (kasite === "vaativa_1" || kasite === "vaativa_2") && {
              perusteluteksti_vaativa: meta_vaativa
            }),
          ...(koodistoUri === KOODISTOT.OIVA_TYOVOIMAKOULUTUS &&
            (koodiArvo === "1" || koodiArvo === "3") && {
              perusteluteksti_tyovoima: meta_tyovoima
            })
        },
        muutosperustelukoodiarvo: null
      });
...
export function handleOsaamislaCheckboxChange(
  event,
  editValue,
  fields,
  isInLupa,
...
export function handleTutkintoKieliCheckboxChange(
  event,
  editValue,
  fields,
  isInLupa,
...
export function handleTutkintokieliSelectChange(
  editValues,
  fields,
  isInLupa,
  tutkinto,
