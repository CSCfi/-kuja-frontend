...
class Section extends Component {
  render() {
    const { heading, target, maaraykset } = this.props
    ...
    if (maaraykset) {
      ...
      if (Number(target) === KOHTEET.TUTKINNOT) {
        ...
        let alat = this.sortTutkinnot(tutkinnot);
        muutMaaraykset = _.sortBy(muutMaaraykset, (muuMaarays) => { return muuMaarays.indeksi })

        const diaari = this.props.diaarinumero.replace(/\//g, '-')
        ...
        return (
          <SectionWrapper>
            <Span>{`${target}.`}</Span>
            <H3>{`${heading}`}</H3>
            <MuutosLink to={`/jarjestajat/${this.props.ytunnus}/hakemukset-ja-paatokset/${diaari}`} diaarinumero={this.props.diaarinumero}>Hae muutosta</MuutosLink>
            <div>
              <Tutkinnot>
